export function highlightLog(log) {
  ...
  bashColors.map(function(code){
    log = log.replace(code[0], code[1])
  })

  if (log !== undefined){
    log =  log.replace(timestampRegex, '<time class="time">$2</time>')
    log = log.replace(failRegex, '<span class="red">$1</span>')
    log = log.replace(succRegex, '<span class="green">$1</span>')
  }
  return log;
};
