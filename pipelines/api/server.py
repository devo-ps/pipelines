...
from tornado.web import (RequestHandler, Application, url, HTTPError,
                         authenticated, StaticFileHandler)
...
def _authenticate_user(auth_settings, username, password):
    if username != auth_settings['username'] or password != auth_settings[
            'password']:
        return False
    return True
        ...
        if self.request.body:
            try:
                ...
                if isinstance(json_body, dict):
                    ...
                    log.warning('JSON content not dict, it was %s' %
                                type(json_body))
            except ValueError as e:
                log.warning('ValueError processing webhook content: %s' % e)
                pass

        return _run_pipeline(self,
                             workspace,
                             wh_context['slug'],
                             params=params)
    ...
    def get(self, pipeline_slug):
        ...
        with open(yaml_filepath) as f:
                ...
                if os.path.isfile(config_path):
                    with open(config_path) as wh_file:
                        ...
                        if 'webhooks' not in pipeline_config:
                            pipeline_config['webhooks'] = {}
                if not pipeline_config.get(
                        'slackbot') or not pipeline_config.get('slackbot').get(
                            'public_slug'):
                    if 'slackbot' not in pipeline_config or not isinstance(
                            pipeline_config['slackbot'], dict):
                        pipeline_config['slackbot'] = {}

                    pipeline_config['slackbot']['public_slug'] = str(uuid4())

                for index, trigger in enumerate(
                        pipeline_def.get('triggers', [])):
                    if trigger.get('type') == 'webhook':
                        ...
                        wh_id = _get_webhook_id(wh_identifier,
                                                pipeline_config['webhooks'])
            ...
            self.write(
                json.dumps(
                    {
                        'triggers': pipeline_def.get('triggers', []),
                        'slackbot': pipeline_config['slackbot']['public_slug']
                    },
                    indent=2))
            self.finish()
...
class LoginHandler(PipelinesRequestHandler):
    ...
    def get(self):
        ...
        login_type = self.settings['auth'].get(
            'type') if self.settings['auth'] else None
        self.render('templates/login.html',
                    error_msg=None,
                    login_type=login_type)

    def post(self):
        if _authenticate_user(self.settings['auth'],
                              self.get_argument('username'),
                              self.get_argument('password')):
            self.set_secure_cookie(
                "user",
                tornado.escape.json_encode(self.get_argument("username")))
            print('set secure cookie')
            self.redirect("/index.html")
        else:
            self.render('templates/login.html',
                        error_msg='Wrong username or password',
                        login_type=self.settings['auth'].get('type')
                        if self.settings['auth'] else None)
    ...
    def get(self):
        ...
        for path in _file_iterator(workspace, extensions=PIPELINES_EXT):
            ...
            try:
                ...
                log.error(
                    'Can not read pipelines, file missing: {}'.format(path))
                continue
            ...
            try:
                ...
                log.error(
                    'Skipping pipeline. Could not load yaml for: {}'.format(
                        path))
                continue
        ...
        if os.path.isfile(run_status_path):
            with open(run_status_path) as f:
                try:
                    ...
                    log.warning(
                        'Failed to parse json file. Skipping. File: %s' %
                        run_status_path)
        else:
...
class GetLogsHandler(PipelinesRequestHandler):
    ...
    def get(self, pipeline_slug, task_id):
        ...
        logfile_path = os.path.join(workspace, pipeline_slug, task_id,
                                    'output.log')
...
class GetStatusHandler(PipelinesRequestHandler):
    ...
    def get(self, pipeline_slug, task_id):
        ...
        statusfile_path = os.path.join(workspace, pipeline_slug, task_id,
                                       'status.json')
...
def _get_auth_dict(auth_settings):
    ...
    if auth_settings[0] == 'gh':
        return {'type': 'gh', 'teams': auth_settings[1]}
...
def make_app(cookie_secret=None,
    ...
    endpoints = [
        ...
        url(r"/api/pipelines/({slug})/".format(slug=slug_regexp),
            GetPipelineHandler),
        url(r"/api/pipelines/({slug})/run".format(slug=slug_regexp),
            RunPipelineHandler),
        url(
            r"/api/pipelines/({slug})/({slug})/status".format(
                slug=slug_regexp), GetStatusHandler),
        url(r"/api/pipelines/({slug})/({slug})/log".format(slug=slug_regexp),
            GetLogsHandler),
        url(r"/api/pipelines/({slug})/triggers".format(slug=slug_regexp),
            GetTriggersHandler),
        url(r"/api/webhook/({slug})".format(slug=slug_regexp), WebhookHandler),
        url(r"/api/slackbot/({slug})".format(slug=slug_regexp),
            SlackbotHandler),
        (r"/login", LoginHandler),
        (r'/(.*)', AuthStaticFileHandler, {
    ...
    if auth_dict and auth_dict.get('type') == 'gh':
        endpoints.insert(
            len(endpoints) - 1, (r"/ghauth", GithubOAuth2LoginHandler)),
