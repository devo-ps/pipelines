...
def load_slackbot_commands(workspace_path):
    ...
    for slug, pipeline_def in walk_pipelines(workspace_path):
        slackbot_confs = filter(lambda d: d.get('type') == 'slackbot',
        ...
        for slackbot_conf in slackbot_confs:
            if slackbot_conf.get('command'):
                ...
                log.debug(
                    'Slackbot config is missing "command" parameter, skipping, slug=',
                    slug, slackbot_conf)
...
def _read_slackbot_state(workspace):
    ...
    with open(conf_path) as f:
        try:
            ...
            slackbot_slug = pipeline_state.get('slackbot',
                                               {}).get('public_slug')
...
class SlackbotHandler(RequestHandler):
    ...
    def _parse_body_args(self, body):
        ...
        parse_body_arguments('application/x-www-form-urlencoded',
                             self.request.body, args, {})
        # command, environment, component, branch = args['text'].split(' ')
        command_args = args['text'][0].split(' ')

        return {
            'response_url':
            args.get('response_url')[0],
            'user':
            args.get('user_name', [''])[0],
            'channel':
            args['channel_name'][0]
            if len(args.get('channel_name', [])) else None,
            'command':
            command_args[0] if len(command_args) else '',
            'args':
            args['text'][0].split(' '),
            'slash_command':
            args['command'][0]
    ...
    def usage(self, commands, slash_command):
        commands = {
            c['command']: '%s %s %s' % (slash_command, c['command'], ' '.join(
                ['(%s)' % arg for arg in c['arguments'][1:]]))
    ...
    def post(self, slug):
        ...
        if slug != pipelines_slug:
            raise HTTPError(
                404, 'Slug doesn\'t match %s %s' % (slug, pipelines_slug))
        ...
        log.debug('Generate slackbot commands. Found %s slackbot commands' %
                  len(slackbot_commands.keys()))

        body_args = {}
        parse_body_arguments('application/x-www-form-urlencoded',
                             self.request.body, body_args, {})
        ...
        pipeline_args = {'trigger': 'slackbot', 'slakbot_args': command_args}

        # Map args
        named_slackbot_args = {}
        if command_config.get('arguments'):
            if len(command_config.get('arguments')) != len(
                    command_args['args']):
                log.warn('Incorrect amount of arguments %s, expecting %s' %
                         (len(command_args['args']),
                          len(command_config.get('arguments'))))
                return 'Incorrect amount of arguments %s, expecting %s' % (len(
                    command_args['args']), len(
                        command_config.get('arguments')))
        ...
        # For legacy reasons
        pipeline_args['webhook_content'] = named_slackbot_args
        ...
        def response_fn(handler, task_id):
            handler.write({
                ...
                'text':
                '%s triggered by %s' %
                (command_args['command'], command_args.get('user')),
                'attachments': [{
                    "text":
                    yaml.dump(named_slackbot_args, default_flow_style=False)
                }]
            })
            handler.finish()
